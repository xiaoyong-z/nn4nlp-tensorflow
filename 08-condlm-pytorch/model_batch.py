...
class encoder(torch.nn.Module):
    ...
    def forward(self, source, src_length=None, hidden=None):
        ...
        if hidden is None:
            if self.use_cuda:
                h0 = Variable(torch.zeros(self.num_layer, batch_size, self.hidden_dim).cuda())
                c0 = Variable(torch.zeros(self.num_layer, batch_size, self.hidden_dim).cuda())
            else:
                h0 = Variable(torch.zeros(self.num_layer, batch_size, self.hidden_dim))
                c0 = Variable(torch.zeros(self.num_layer, batch_size, self.hidden_dim))
            hidden = (h0, c0)
    ...
    def forward(self, source, target=None, masks=None):
        ...
        if target is not None:
            ...
            for i in range(target.shape[1] - 1):
                tar = target[:,i].view(batch_size, -1)
                output, hidden = self.decoder(tar, hidden)
                loss = criterion(output.squeeze(1), target[:,i+1])
                loss = masks[:,i].float() * loss
                loss = torch.sum(loss)
                total_loss.append(loss)
        else:
            enc_h = self.encoder(source.unsqueeze(0))
            hidden = enc_h

            trg_word = torch.tensor(self.trg_soi).type(type)
            i = 0
            for i in range(self.max):
                output, hidden = self.decoder(trg_word.unsqueeze(0).unsqueeze(0), hidden)
                trg_word = torch.argmax(output[0], dim=1)[0]
